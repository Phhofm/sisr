# aether_arch.py for traiNNer-redux
# This file defines the AetherNet architecture for use with the traiNNer-redux framework.
...
# traiNNer-redux-specific imports
try:
    from traiNNer.utils.registry import ARCH_REGISTRY
except ImportError:
    # Dummy registry for standalone testing or if traiNNer-redux path is not set up
    print("Warning: traiNNer.utils.registry not found. Using dummy registry.")
    class DummyRegistry:
...
class ReparamLargeKernelConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, groups, small_kernel, fused_init=False):
        ...
        if self.fused:
            # If initializing as fused, directly create the fused_conv
            self.fused_conv = nn.Conv2d(
                in_channels, out_channels, kernel_size, stride, self.padding, groups=groups, bias=True
            )
            # Weights/biases for fused_conv will be loaded from the state_dict
        else:
            # Normal (unfused) initialization for training or pre-fusion state
    ...
    def fuse(self):
        ...
        # Remove original unfused parameters
...
class aether(nn.Module):
    ...
    def __init__(
        ...
        in_chans=3,
        embed_dim=128,
        depths=(6, 6, 6, 6), # Default to medium variant depths
        mlp_ratio=2.0,
        drop_rate=0.,
        drop_path_rate=0.1,
        lk_kernel=11,
...
# --- Model Variants (registered for traiNNer-redux) ---

@ARCH_REGISTRY.register()
def aether_small(**kwargs):
    """AetherNet Small variant, registered with traiNNer-redux's ARCH_REGISTRY."""
    return aether(
        embed_dim=96,
        depths=(4, 4, 4, 4),
        mlp_ratio=2.0,
        **kwargs # Pass along other parameters from traiNNer-redux config
    )

@ARCH_REGISTRY.register()
def aether_medium(**kwargs):
    """AetherNet Medium variant, registered with traiNNer-redux's ARCH_REGISTRY."""
    return aether(
        embed_dim=128,
        depths=(6, 6, 6, 6, 6, 6),
        mlp_ratio=2.0,
...
def aether_large(**kwargs):
    """AetherNet Large variant, registered with traiNNer-redux's ARCH_REGISTRY."""
    return aether(
        embed_dim=180,
